...
actually present in this project (`rag_agent`'s embeddings always; `lg_agent`'s
BM25 index if scaffolded — see `pipelines/run.py`'s `BACKENDS` list).
`adk_agent` has no entry of its own since it routes to `rag_agent` via MCP and
...
1. The backend's own search function (`core.index.search` for `lg_agent`'s BM25
...
each backend's vector DB to exist first — run `make corpus-ingest` (`lg_agent`)
and/or `make rag-corpus-ingest` (`rag_agent`) if you see a "vector DB not
