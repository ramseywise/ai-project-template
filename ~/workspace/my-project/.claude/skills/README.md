...
| `akira` | Proactive quality check — "what did we miss?" | Runs in-session (kiyoko questions, dao fixes) + spawns the vendored `akira-scan` agent for the kaneda scan | No |
| `grow-companion` | A durable collaboration pattern shifted mid-session | Transforms `.claude/docs/companion/collaboration.md` — not append-only logging | No |
| `dream` | Companion doc or memory index feels stale | Audits staleness, does light consolidation | No |
