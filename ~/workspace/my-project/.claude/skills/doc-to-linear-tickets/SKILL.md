...
1. Identify the **workstream** (top-level grouping, e.g. "Data Ingestion")
...
Check the Linear team's existing label taxonomy before creating tickets — list current labels and their colors, and reuse the ones that already cover the workstream. Only create a new label if none of the existing ones fit; check with the user if unsure which label applies to a borderline ticket.
...
- "Create Linear tickets for the Data Ingestion initiative"
